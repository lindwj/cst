...
<link href="<%=path%>/resource/common/reset.css" rel="stylesheet"
	type="text/css" />
<link href="<%=path%>/resource/css/market.css"
...
function pass(obj){
	...
	$("#surebtn").click(function(){
	    ...
	    input1.type = "text";  
	    input1.name = "marketRemark";  
	    input1.value =remark ;  
	    form.appendChild(input1);  
	    form.method = "POST";  
	    form.action = "<%=path%>/Ordergoods/passMarketList.do";  
	    form.submit();  
...
function refuse(obj){
	...
	$("#surebtn").click(function(){
	    ...
	    form.appendChild(input1);  
	    form.method = "POST";  
	    form.action = "<%=path%>/Ordergoods/refuseMarketList.do";  
	    form.submit();  
	    document.body.removeChild(form);  
	})
	//取消事件
...
function success(obj){
	...
	$("#surebtn1").click(function(){
	    ...
	    form.appendChild(input); 
	    form.method = "POST";  
	    form.action = "<%=path%>/Ordergoods/success.do";  
	    form.submit();  
	    document.body.removeChild(form);  
	})
	//取消事件
...
<div class="black" id="hidebox">
    <div class="fadebox">
        ...
        <textarea name="marketRemark" id="remark"></textarea>
        </div>
        <div class="clear"></div>
...
<div class="container">
        ...
        <div class="content">
        	<form name="shareForm" id="shareForm" method="post" action="<%=path%>/Ordergoods/searchMarketList.do">
                ...
                <table class='table2'>
                    ...
                    <tr>
                        ...
                        <td class='col8' valign='top'>
                        	<c:if test="${item.myStatus==1}">
                            	<input class='changebtn' type='button' value='通过' onclick="pass(this)" data-id="${item.ordergoodsId}">
                            	<input class='changebtn' type='button' value='拒绝' id="refusebtn" onclick="refuse(this)" data-id="${item.ordergoodsId}">
                            </c:if>
                            <input class='changebtn' type='button' value='订单详情' data-id="${item.code}"onclick="detail(this)">
                            <c:if test="${item.status==3}">
                            	<input class='changebtn' type='button' onclick="success(this)" data-id="${item.ordergoodsId}" value='订单完成'>
                            </c:if>
                        </td>
                    </tr>
                    </c:if>
