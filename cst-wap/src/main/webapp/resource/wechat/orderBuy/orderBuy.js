...
$(function(){
	...
	var prouuid=window.location.href.split("?productUuid=")[1];
	var puuid=prouuid.split("&")[0];
    ...
    function getDefault(info){
    	...
    	$(".order_state").click(function(){
    		window.location.href=addressList_path+"?"+orders;
        })
    }
    ...
    function orderUp(info) {
        ...
        $(".buy").click(function(){
        	var note="";
        	note=$(".p3").val();
        	detailList=detailList+"&note="+note+"&totalFromBdh="+info.totalFromBdh+"&totalFee="+info.totalFee+"&shopId="+info.shopId+"&shopName="+info.shopName+"&receiveName="+man+"&goodsAddressUuid="+uuid+"&receiveAddress="+addressAll+"&receiveMobile="+phone;
        	$.ajax({
        		type: 'POST',
                url:saveNow_url,
                dataType : "json" ,
