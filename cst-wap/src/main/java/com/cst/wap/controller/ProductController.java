...
public class ProductController {
	...
	@ResponseBody
	public Product cartProductListPage(Product product,HttpServletRequest request,HttpServletResponse response, Model model) throws Exception {
		...
		if(subject.isAuthenticated()){
			...
			productsList = productService.getCartdbProductListPage(product);
			product.setProducts(productsList);
			return product;
		}else{
			//cookie 获取数据
			...
			if(bdhcookie!=null){
				...
				if(value.length()>5){
					...
					productsList = productService.getCartcookieProductListPage(product);
	...
	public Product productDetailSvc(Product product,HttpServletRequest request) throws Exception {
		...
		if(sub.isAuthenticated()){
			...
			//获取购物车内商品
			ps.setCreateByAdm(user.getUserId());
			List<Product> productsLists = productService.getCartdbProductListPage(ps);
			int i=0;
			for(Product p:productsLists){
				i=i+p.getCapacity();
			}
			product.setCartAll(i);
		}else{
			//cookie 获取数据
			Cookie bdhcookie= Common.getCookieByName(request, "bdhcookie");
			if(bdhcookie!=null){
				...
				if(value.length()>5){
					...
					//查询商品
					pp.setProductUuid(uuids.toString());
					List<Product> psList = productService.getCartcookieProductListPage(pp);
					int j=0;
					//将之前 计算的 数量值  赋值给 list
					for(Map.Entry<String,Integer> kvmap : kv.entrySet()){
						for(Product prd : psList){
							if(kvmap.getKey().equals(prd.getProductUuid())){
								j=j+kvmap.getValue();
								prd.setCapacity(kvmap.getValue());
							}
					...
					product.setCartAll(j);
				}else{
					product.setCartAll(0);;
				}
			}else{
				product.setCartAll(0);
			}
