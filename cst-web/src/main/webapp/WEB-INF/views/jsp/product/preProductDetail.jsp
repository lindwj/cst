...
var myreg = /^(((13[0-9]{1})|(14[0-9]{1})|(17[0]{1})|(15[0-3]{1})|(15[5-9]{1})|(18[0-9]{1}))+\d{8})$/;
	...
	if(applyPhone.length != 11 || !myreg.test(applyPhone)){
		ProductDetail.popModal("请输入有效的手机号码！");
		return;
	}
	
	if(applyName.length > 6 || applyName.length <= 0 || applyPhone == null){
		ProductDetail.popModal("姓名必填，不超过6个汉字。");
		return;
	}
	
	
	jQuery.ajax({
		  url: "<%=path%>/preOrders/preOrdersAdd.do",
						cache : false,
						 type:"POST",
						data : "applyName=" + applyName+"&applyPhone="+applyPhone,
						success : function(data) {
							if(eval(data) == 'ok'){
								$("#addr-frame").addClass("display-none")
							    $("#frame-mask").addClass("display-none");
								ProductDetail.popModal("提交成功，贵宾专员会主动联系您。");
					...
					${goodsAddress.provincestr}&nbsp;${goodsAddress.citystr}&nbsp;${goodsAddress.districtstr}
								...
								<div class="range">
									<dl>
										...
										<dd>
											<div class="range-content"><c:if test="${product.dowhat != '0' }">由 无锡市通江大道店 销售和发货，并提供售后服务。</c:if><c:if test="${product.dowhat == '0' }">本地区暂不支持配送</c:if></div>
										</dd>
									</dl>
								</div>
								...
								<div class="action">
									<div class="buy">
										<a id="link-buy" href="javascript:void(0)"
											title="点击此按钮，到下一步确认购买信息。" role="button" >购买申请</a>
...
<div class="dialog display-none" id="addr-frame">
    <div class="dialog-content">
        ...
        <div class="dialog-title">订单预定</div>
        <div class="dialog-content">

            <div>
                <label>联系人：</label>
                <input name="applyName" id="applyName" type="text" class="input-default" placeholder="输入联系人"  style="width: 150px">
            </div>
            <div>
                <label>手机号码：</label>
                <input name="applyPhone" id="applyPhone" type="text" class="input-default" placeholder="输入手机号" style="width: 150px">
            </div>
            <div>
                <label>温馨提醒：</label>
                详情请拨打<span>0510-68791234</span>
            </div>

            <div class="def-address">
                <button type="button" class="submit-btn" id="submit-btn" onclick="preorders()">提交</button>
            </div>
