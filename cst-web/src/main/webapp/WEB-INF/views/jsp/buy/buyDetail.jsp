...
function saveAds(){
	...
	jQuery.ajax({
		  url: "<%=path%>/goodsAddress/goodsAddressAdd.do",
						cache : false,
						 type:"POST",
						data : "province=" + p+"&city="+c+"&district="+d+"&address="+address+"&name="+perName+"&mobile="+phone+"&isDefault="+num,
						success : function(data) {
							if(data.length>=10){
    ...
    <div class="address">
            ...
            <li class="clearfix <c:if test="${1 == goodsAddress.isDefault}">selected</c:if>" id="sel_${goodsAddress.goodsAddressUuid}" data-addressid="${goodsAddress.goodsAddressUuid}">
                    ...
                    <div class="address-info">
                        ...
                        <a class="set-default <c:if test="${1 == goodsAddress.isDefault}">hidden</c:if>" href="javascript:void(0);" id="setDef_${goodsAddress.goodsAddressUuid}" onclick="def(this)" gid="${goodsAddress.goodsAddressUuid}">设置为默认收货地址</a>
        ...
        <div class="order clearfix">
            ...
            <%int i=0; %>
                ...
                <div class="item clearfix">
                    ...
                    <input id="${ordersDetail.productUuid}productUuid" name="ordersDetailList[<%=i %>].productUuid" type="hidden" value="${ordersDetail.productUuid}" />
                    <input id="pic" name="ordersDetailList[<%=i %>].pic" type="hidden" value="${ordersDetail.pic}" />
                        ...
                        <input name="ordersDetailList[<%=i %>].capacity" id="${ordersDetail.productUuid}column_num" type="hidden" value="${ordersDetail.capacity}" />
        ...
        <div><p id="Error" style="color: #c81623;padding-left: 105px"></p></div>
            <div class="form-item">
                ...
                <select
							...
							<option value="">请选择</option>
						</select> <select style="display: none;" name="street" class="inpput-address " id="selstreet">
							<option value="">请选择</option>
						</select>
